namespace StabilityMatrix.Core.Models.Api.Comfy.NodeTypes;

public class LatentNodeConnection : NodeConnectionBase;

public class VAENodeConnection : NodeConnectionBase;

public class ImageNodeConnection : NodeConnectionBase;

public class ControlEmbedsNodeConnection : NodeConnectionBase;

public class ImageWidthNodeConnection : NodeConnectionBase;

public class ImageHeightNodeConnection : NodeConnectionBase;

public class ImagePathNodeConnection : NodeConnectionBase;

public class LoraNodeConnection : NodeConnectionBase;

public class TextNodeConnection : NodeConnectionBase;

public class NoiseLatentNodeConnection : NodeConnectionBase;

public class NoiseCondNodeConnection : NodeConnectionBase;

public class NoiseAugNodeConnection : NodeConnectionBase;

public class NoiseStrengthNodeConnection : NodeConnectionBase;

public class NoiseScheduleNodeConnection : NodeConnectionBase;

public class NoiseMixNodeConnection : NodeConnectionBase;

public class NoiseClipNodeConnection : NodeConnectionBase;

public class GenericNodeConnection : NodeConnectionBase;

public class TextEmbedsNodeConnection : NodeConnectionBase;

public class T5NodeConnection : NodeConnectionBase;

public class ImageEmbedsNodeConnection : NodeConnectionBase;

public class ImageMaskConnection : NodeConnectionBase;

public class UpscaleModelNodeConnection : NodeConnectionBase;

public class ModelNodeConnection : NodeConnectionBase;

public class CompileArgsNodeConnection : NodeConnectionBase;

public class BlockSwapNodeConnection : NodeConnectionBase;

public class ConditioningNodeConnection : NodeConnectionBase;

public class FetaArgsNodeConnection : NodeConnectionBase;

public class ClipNodeConnection : NodeConnectionBase;

public class ControlNetNodeConnection : NodeConnectionBase;

public class ClipVisionNodeConnection : NodeConnectionBase;

public class ClipVisionOutputNodeConnection : NodeConnectionBase;

public class SamplerNodeConnection : NodeConnectionBase;

public class SigmasNodeConnection : NodeConnectionBase;

public class StringNodeConnection : NodeConnectionBase;

public class BboxDetectorNodeConnection : NodeConnectionBase;

public class SegmDetectorNodeConnection : NodeConnectionBase;

public class SamModelNodeConnection : NodeConnectionBase;

public class GuiderNodeConnection : NodeConnectionBase;

public class NoiseNodeConnection : NodeConnectionBase;
